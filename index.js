...
app.get('/accessAll', (req,res)=>{
    if (req.session.loggedin){
        db.query("SELECT role FROM user WHERE username = ?", [req.session.username], function(err, result ,fields){
    ...
    if (req.session.loggedin){
        db.query("SELECT role FROM user WHERE username = ?", [req.session.username], function(err, result ,fields){
            if (result[0].role == "ceo"){
                    res.send("Welcome "+req.session.username+" , You can change your employees role please click button to next step")
            } else {
                res.status(403);
            }
            res.end();
...
// app.get('/page/grades', authPage(['teacher','admin']), (req,res)=>{
//     res.json({
//         ant:100,
//         bee:80,
//         cat:25,
